...
def admin():
    ...
    with root_with_password_disabled() as context:
        ...
        context.client.call("user.setup_local_administrator", "admin", "password")
        call("system.info", client_kwargs=dict(auth=("admin", "password")))
...
def test_installer_admin_has_local_administrator_privilege(admin):
    with client(auth=("admin", "password")) as c:
        c.call("system.info")
